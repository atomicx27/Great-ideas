...
${compiledCritiques}`;
    ...
    async function askLLM(provider, apiKey, model, systemPrompt, userMessage) {
        if (provider === 'openai') return await fetchOpenAI(apiKey, model, systemPrompt, userMessage, { temperature: 0.4 });
        if (provider === 'anthropic') return await fetchAnthropic(apiKey, model, systemPrompt, userMessage, { max_tokens: 4096 });
